    ...
    <div className="week-view">
      ...
      <div className="overflow-x-auto border border-gray-200 rounded-xl shadow-sm">
        <table className="min-w-full border-collapse">
          <thead>
            <tr>
              <th className="w-20 p-2 text-xs bg-gray-50 border-r text-gray-600">Time</th>
              {weekDays.map((day, i) => (
                <th key={i} className="p-2 text-xs bg-gray-50 border-l border-gray-200 text-gray-700">
          ...
          <tbody>
            {timeSlots.map((slot, i) => (
              <tr key={i} className="border-t hover:bg-gray-50 transition">
                <td className="p-2 text-xs text-gray-500 border-r">{slot.label}</td>
                {weekDays.map((day, j) => (
